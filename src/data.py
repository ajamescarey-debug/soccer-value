...
Computation:
  - For each bet, we record `taken_odds` when we flag the bet.
  - At kickoff (or as close as we can get), we record `closing_odds`
    from the same book or a sharp reference book (Pinnacle preferred).
  - CLV per bet = (taken_odds / closing_odds) - 1, expressed as a percentage.
  - Average CLV across all bets is the headline number.
