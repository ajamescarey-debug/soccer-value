...
the implied probabilities sum to 1.053 (= 1/1.90 + 1/1.90), not 1.0.
That 5.3% overround is the book's edge. To compare the market's
*opinion* to our model, we have to strip it.

Two methods implemented:
  1. Proportional (basic) — divide each implied prob by their sum.
     Simple, but slightly mis-shapes the distribution at the extremes.
  2. Shin's method — assumes a population of insider bettors and solves
     for the true probability accordingly. More accurate, especially
     for skewed markets. Standard in academic literature.

For 2-way markets (Over/Under, BTTS), proportional is fine — the
distortion is tiny. We default to it for clarity.
"""
...
def devig_proportional(odds_list: List[float]) -> List[float]:
    ...
    For a 2-way market this is: p_true = (1/odds) / sum(1/odds_all).
    """
...
def margin(odds_list: List[float]) -> float:
    ...
    Useful for filtering out bookmakers with absurdly bad lines.
    """
